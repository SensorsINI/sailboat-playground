...
- Removed the heavy JSONL state logger—profiling showed it slowed the sim down without helping day-to-day tuning.
...
- Added analytic NACA0006 foil polars (`foils/naca0006.csv`, `foils/naca0006_rudder.csv`) generated for Re≈7×10⁵ to use as baseline keel/rudder data.
- Modelled hull lateral drag explicitly:
  - New config parameters `hull_side_area`, `hull_side_coefficient`, and `hull_side_center_from_com`.
  - `Manager.step()` now projects the apparent current into body axes, applies a resisting side force, and adds the resulting lever-arm torque to yaw dynamics.
